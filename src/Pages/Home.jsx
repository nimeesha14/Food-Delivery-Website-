    ...
    return (
      <div className='bg-slate-200 w-full min-h-screen'>
          ...
          {!input?<div className='flex flex-wrap justify-center items-center gap-6 w-[100%] '>
              {Category.map((item)=>{
                return <div className='h-[120px] w-[110px] bg-white flex flex-col item-start gap-5 p-5 justify-start text-[15px] font-semibold text-gray-600 rounded-lg shadow-xl hover:bg-green-200 cursor-pointer transition-all duration-200 'onClick={()=>filter(item.name)}>
                  {item.icon}
                  {item.name}
                </div>
              })}
          ...
          <div className={`w-full md:w-[40vw] h-[100%] fixed top-0 right-0  bg-white p-5 transition-all duration-500 flex flex-col items-center overflow-auto ${showCard?"translate-x-0":"translate-x-full"}`}>
            <header className='w-[100%] flex justify-between items-center'>
            <span className='text-green-400 text-[18px] font-semibold'>Order Item</span>
            <RxCross2 className='w-[20px] h-[20px] text-green-400 text-[18px] font-semibold cursor-pointer hover:text-green-600' onClick={()=>setShowCard(false)}/>
            </header>
            ...
            <div className='w-full border-t-2 border-b-2 border-gray-400 mt-4 flex flex-col gap-2 p-3'>
            ...
            <div className='w-full flex justify-between items-center p-5'>
              ...
              <button className='w-[80%] p-2 mt-3 bg-green-500 rounded-lg text-white hover:bg-green-400 transition-all'onClick={()=>{toast.success("Oeder place")}}>Place Order</button>
